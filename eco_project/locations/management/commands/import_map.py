...
def get_mesh_data_from_file() -> dict[str, dict[str, Any]]:
    ...
    ), "r", encoding="utf-8") as console_read:
    ...
    for line in console_lines:
            ...
            center_deg = (
                round(
                    ...
                    float(
                        d[0].split(", ")[1]), 5))
            bottom_left_deg = (
                round(float(d[1].split(", ")[0]), 5),
                round(float(d[1].split(", ")[1]), 5),
            )
            top_right_deg = (
                round(float(d[2].split(", ")[0]), 5),
        ...
        if "Tile center in" in line:  # if the line contains the blender coordinates of the chunk
            ...
            # convert to float tuples
            bottom_left_blender = (
                round(float(t[0].split(", ")[0]), 3),
                round(float(t[0].split(", ")[1]), 3),
                round(float(t[0].split(", ")[2]), 3),
            )
            top_right_blender = (
                round(float(t[1].split(", ")[0]), 3),
                round(float(t[1].split(", ")[1]), 3),
                round(float(t[1].split(", ")[2]), 3),
            )
        ...
        if "Imported object" in line:  # if the line contains the name of the chunk
            ...
            data[obj_name] = {
                ...
                "bottom_left_deg": bottom_left_deg,
    ...
    def handle(self, *args, **kwargs) -> None:
        ...
        for name, data in chunk_dict.items():
            ...
            if not os.path.exists(
                    os.path.join(os.getcwd(), "locations/management/commands/exports/",
                                 f"{name}.glb")):
                print(f"File not found: {name}")
                continue

            with open(os.path.join(os.getcwd(), "locations/management/commands/exports/",
                                   f"{name}.glb"), "rb") as f:
                ...
                chunks.append(
                    Map3DChunk(
                        ...
                        file=default_storage.save(
                            f"locations/3d_map_chunks/{name}.glb", file
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Saved {len(chunk_dict)} chunks to database")
        )
        image_path = os.path.join(
            os.getcwd(), "locations/management/commands/heightmap.png"
        )
        ...
        with open(image_path, "rb") as f:
            ...
            img_file = default_storage.save(
                f"locations/camera_z_map/camera_height_map"
                f"{''.join([choice('1234567890ABCDEF') for _ in range(5)])}.png",
                file
            )
        ...
        dummy_instance = (
            Map3DChunk.objects.first()
        )  # This is just to trigger the signal handler
