...
def get_mesh_data_from_file() -> dict[str, dict[str, Any]]:
    ...
    for line in console_lines:
            ...
            center_deg = (
                round(
                    ...
                    float(
                        center[1]), 5))
            bottom_left_deg = (
                round(float(bottom_left[0]), 5),
                round(float(bottom_left[1]), 5),
            )
            top_right_deg = (
                round(float(top_right[0]), 5),
                round(float(top_right[1]), 5),
            )
        ...
        if (
            ...
            # convert to float tuples
            bottom_left_blender = (
                round(float(bottom_left[0]), 3),
                round(float(bottom_left[1]), 3),
                round(float(bottom_left[2]), 3),
            )
            top_right_blender = (
                round(float(top_right[0]), 3),
                round(float(top_right[1]), 3),
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs) -> None:
        ...
        path_to_exports: str = os.path.join(
            os.getcwd(), "locations/management/commands/exports/"
        )
        ...
        for name, data in chunk_dict.items():
            ...
            with open(file_path, "rb") as f:
                ...
                safe_file_path = default_storage.save(
        ...
        letters = "".join([choice("1234567890ABCDEF") for _ in range(5)])
        img_file = None
